...
\noindent
Multiple deep learning models suitable to the three data representation were created. The models used supervised learning, which is defined as a network learning to classify a given input corresponding to a specific output \citep{Goodfellow2016}. The models were designed differently according to its pain maps-representations. The architecture of the two models including morphology- or combined-representation consisted of three convolutional layers, to which a max pooling layer was designed after each convolutional layer. The last four layers were designed as fully connected layers. The architecture of these models is illustrated in fig. \ref{fig:models}.
The final model, including the location-representation was designed with four fully connected layers, which is the last part shown in fig. \ref{fig:models}.
