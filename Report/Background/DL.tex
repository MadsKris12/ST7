...
Deep learning networks are structures of hidden layers, and this requires to choose the activation functions for computing the hidden layer values and to decide whether node could be considered as active or not\citep{Goodfellow2016}. The activation function is a nonlinear function which give neural network nonlinear capabilieties\citep{Bengio2012}. It applied element-wise and can be defined as new output of each node:
>>>>EQUATION<<<<

Sigmoid activation function is one of the most common activation functions which monotonicaly approches at some finite values as $\pm\infty$. Most common examples would be the standard logistic function $\f(x) = 1/(1 + e^−x)$ and hyperbolic tangent $\[f(x) = tanh(x)$ which is shown in figure \ref{fig:Sigmoids}. Symetric hyperbolic tangent is used more often because it converges faster then standard logistic function. \citep{Bengio2012}
...
\begin{figure} [H]
\centering
\includegraphics[width=0.7\textwidth]{figures/Relu}
...
Convolutional, pooling and fully connected layers could be called as core neural network layers, where convolutional and pooling layers also act as hidden layers. In architecture of the network these layers have specific place and functions \citep{Goodfellow2016,LeCun2015}.
...
\begin{figure} [H]
\centering
\includegraphics[width=0.35\textwidth]{figures/Kernel}
