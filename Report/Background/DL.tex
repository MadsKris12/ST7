...
Sigmoid activation function is one of the most common activation functions which monotonicaly approches at some finite values as $\pm\infty$ .\citep{Bengio2012} Most common examples would be the standard logistic function $\f(x) = 1/(1 + e^−x)$ and hyperbolic tangent $\[f(x) = tanh(x)$ which is shown in figure \ref{fig:Sigmoids}.
...
In model neural networks, rectified linear unit or ReLu is recommended to use as a default activation function\citep{Goodfellow2016}.
